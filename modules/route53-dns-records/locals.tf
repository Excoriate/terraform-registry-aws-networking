locals {
  ...
  */
  ...
  dns_alias_normalized = !local.is_dns_alias_enabled ? [] : [
    for record in var.record_type_alias_config : {
      ...
      alias = record["alias_target_config"] == null ? {} : {
        name                   = record["alias_target_config"]["target_dns_name"] == null ? null : trimspace(record["alias_target_config"]["target_dns_name"])
        zone_id                = record["alias_target_config"]["target_zone_id"] == null ? null : trimspace(record["alias_target_config"]["target_zone_id"])
  ...
  dns_alias_to_create = !local.is_dns_alias_enabled ? {} : {
    for record in local.dns_alias_normalized : record["name"] => {
      name                                = record["name"]
      zone_name                           = record["zone_name"]
      zone_id                             = record["zone_id"]
      allow_overwrite                     = record["allow_overwrite"]
      is_zone_lookup_by_zone_id_enabled   = record["is_zone_lookup_by_zone_id_enabled"]
      is_zone_lookup_by_zone_name_enabled = record["is_zone_lookup_by_zone_name_enabled"]
      alias = {
        name                   = record["alias"]["name"]
        zone_id                = record["alias"]["zone_id"]
        evaluate_target_health = record["alias"]["evaluate_target_health"]
      }
    }
  }
}
